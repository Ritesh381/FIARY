...
export default function FinanceConfig({ categories, onCategoriesUpdate }) {
  ...
  const handleAddCategory = async () => {
    ...
    setLoading(true);
    try {
      await apiFinanceConfig.createCategory({ name: newCategoryName, isExpense: true });
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <button
          ...
          className="px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white rounded-lg font-semibold flex items-center gap-2 transition-colors"
          disabled={loading}
        >
          <Plus size={20} />
          Add New Category
