...
const GlassCard = ({ children, className = "", ...props }) => (
  <div
    {...props}
    className={`bg-white/10 backdrop-blur-lg border border-gray-700 rounded-2xl shadow-2xl ${className}`}
  >
    {children}
  </div>
);
...
const formatAmount = (amount) => {
  ...
  if (typeof amount === "string" || typeof amount === "number") {
    // Ensure that even if the amount is passed as a string from the form, it's displayed correctly
...
const getSubcategories = (categories, categoryId) => {
  // Categories are expected to be the fully populated list from Redux
...
export default function FinanceEditModal() {
  ...
  useEffect(() => {
    if (selectedEntry) {
      // Normalize amounts and IDs for form fields
      ...
      setFormData({
        ...
        when: toDatetimeLocal(selectedEntry.when), // Updated to use datetime converter
  ...
  const handleChange = (e) => {
    ...
    if (name === "category_id") {
      ...
      // Type change is typically disabled in edit, but handle defensively
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    const payload = {
      ...
      sub_category_id: formData.sub_category_id || null, // Ensure null if empty

      // Pass names for backend backup (as per schema)
  ...
  const confirmationSignature = `DELETE ${formData.type.toUpperCase()} ENTRY: ₹${formatAmount(
    formData.amount
  )} on ${formData.when}`;
  ...
  return (
    <div
      id="finance-edit-modal-backdrop" // <-- BUG FIX: Added unique ID
          ...
          <div className="bg-white/10 backdrop-blur-lg border border-red-500 rounded-2xl p-6 md:p-8 max-w-lg w-full shadow-2xl">
            ...
            <div className="flex justify-end space-x-4 mt-6">
              ...
              <button
                ...
                disabled={
                  deleteConfirmInput !== confirmationSignature || deleting
                }
      ...
      >
        ...
        <form onSubmit={handleUpdateSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-4 p-1 rounded-lg border border-gray-700 opacity-70 cursor-not-allowed">
              <div
                className={`flex-1 py-2 rounded-lg font-medium text-sm md:text-base flex items-center justify-center gap-2 ${
          ...
          {currentSubcategories.length > 0 && (
            <div>
              <label
                ...
                className="block text-sm font-medium text-gray-300 mb-1 flex items-center gap-2"
              >
                <Tag size={16} /> Subcategory (optional)
              </label>
              <select
                id="sub_category_id"
          ...
          <div>
            <label
              ...
              className="block text-sm font-medium text-gray-300 mb-1 flex items-center gap-2"
            >
              <NotepadText size={16} /> Note (optional)
            </label>
            <textarea
              id="note"
              name="note"
