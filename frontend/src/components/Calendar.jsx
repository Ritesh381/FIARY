...
const Calendar = ({
  ...
  return (
    <div className="p-2 sm:p-4 lg:p-6 bg-gray-900 text-white rounded-lg shadow-xl font-urbane w-full">
      {/* Month and Year Selectors */}
      <div className="flex flex-col xs:flex-row gap-2 xs:gap-4 justify-between items-stretch xs:items-center mb-4">
        <select
          value={currentDate.getMonth()}
          onChange={handleMonthChange}
          className="flex-1 bg-gray-800 text-white py-2 px-3 text-xs sm:text-sm lg:text-base rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none cursor-pointer"
        >
          {getMonths().map((month) => (
      ...
      <div className="grid grid-cols-7 text-center text-gray-400 mb-2">
        {dayNames.map((day) => (
          <div key={day} className="text-[10px] xs:text-xs sm:text-sm lg:text-base font-medium">
