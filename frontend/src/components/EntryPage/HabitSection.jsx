...
const HabitSection = ({ habitsData, userHabits, entryDate, handleEntryChange }) => {

    const handleHabitToggle = useCallback((habit, currentEntry) => {
        const isDone = currentEntry?.done || false;
        
        // Dispatch the updateHabitEntry action type via the universal handler
        handleEntryChange("habits", null, { 
            habitId: habit._id,
            entry: { done: !isDone, date: entryDate },
        });
    }, [entryDate, handleEntryChange]);

    const handleHabitNotesChange = useCallback((habit, newNotes) => {
        // Dispatch the updateHabitEntry action type via the universal handler
        handleEntryChange("habits", null, { 
            habitId: habit._id,
    ...
    // Robustness fix: Ensure habitsData is an array
            ...
            <div className="space-y-2 md:space-y-3">
                {userHabits.length === 0 ? (
                    ...
                    userHabits.map((habit) => {
                        // Use safeHabitsData for finding
