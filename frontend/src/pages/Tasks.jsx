...
const PriorityPill = ({ priority }) => {
  ...
  return (
    <span
      className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${classes}`}
    >
      {p.toUpperCase()}
    </span>
  );
...
const TaskItem = ({ task, onEdit, onComplete }) => {
  ...
  const timeRemaining = useMemo(() => {
    ...
    if (hours > 24)
      return {
        text: `Due: ${taskExpiresAt.toLocaleDateString()}`,
        color: "text-gray-400",
      };
  ...
  if (isExpired) {
    ...
    statusText = {
      text: task.isActive ? "Active Template" : "Paused Template",
      color: "text-teal-400",
    };
    ...
    statusText = {
      text: task.category || "One-Time Task",
      color: "text-gray-500",
    };
  }
        ...
        <div className="min-w-0">
          <p
            className={`font-medium truncate ${
              isCompleted || isExpired ? "text-gray-500" : "text-white"
            }`}
          >
            {task.title}
          </p>
...
export default function TasksPage() {
  ...
  useEffect(() => {
    dispatch(
      setNavItems([
        { id: 0, type: "link", name: "Pending Tasks", link: "/tasks" },
        {
          id: 1,
          type: "link",
  ...
  const totalActiveTasks =
    categorizedTodos.pending.length + categorizedTodos.repeating.length;
    ...
    <div className="min-h-screen max-w-4xl mx-auto">
      ...
      {isFormModalOpen && (
        <TaskFormModal
          ...
          onClose={() => dispatch(toggleFormModal(false))}
        />
      )}
